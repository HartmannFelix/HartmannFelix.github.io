var clicky_obj = clicky_obj || (function() {
    ...
    function _ins() {
        this.sitekeys = [];
        var _self = this, site_ids = [], pageviews_fired = [], monitors = 0, setup = 0, ossassets = 0, ossdata = 0;
        ...
        this.custom_data = function() {
            var data = {}, keys = clicky_custom.visitor_keys_cookie || ['username', 'name', 'email'], l = keys.length;
            for (var i = 0; i < l; i++) {
                var key = keys[i], temp = '';
            ...
            if (clicky_custom.visitor) {
                for (var i in clicky_custom.visitor) {
                    if (clicky_custom.visitor.hasOwnProperty && clicky_custom.visitor.hasOwnProperty(i))
                        if (!data[i])
                            data[i] = clicky_custom.visitor[i];
                }
            }
            if (data) {
                for (var i in data) {
        ...
        this.olark = function(s, v, c, do_pageview) {
            var o = s + ',' + v + ',' + c, c = _self.get_cookie('clicky_olark');
            if (c && c == o) {
                if (do_pageview)
                    _self.pageview(1);
                return;
                ...
                if (c)
                    _self.set_cookie('clicky_olark', c, - 3600);
        ...
        this.pageview = function(only_once, extra) {
            ...
            if (_self.facebook_is_lame(href))
                return;
            if (_self.ref && _self.ref.match(/^https?:\/\/(www\.)?semalt.com/))
                return;
        ...
        this.queue_add = function(o) {
            if (!_self.queue_ok())
                return false;
            if (o.type.match(/pageview|download|outbound/i))
                return false;
        ...
        this.queue_process = function() {
            var q = _self.queue_get();
            try {
                if (q.heatmap.length || q.events.length)
                    _self.queue_reset();
                for (var i = 0, s = ''; i < q.heatmap.length; i++) {
                    var o = q.heatmap[i];
                    if (!o.href ||!o.x ||!o.y ||!o.w)
                        return;
                    s += '&heatmap[]=' + _self.enc(o.href) + '|' + o.x + '|' + o.y + '|' + o.w;
                }
                if (s)
                    _self.beacon('heatmap', s);
                while (q.events.length)
                    _self.beacon('', q.events.shift());
            } catch (e) {
                if (_self.debug)
                    console.log(e);
            }
        ...
        this.heatmap_xy = function(e) {
            ...
            if (e.pageX) {
                ...
                return;
            var w = _self.doc_wh(), href = _self.get_href();
        ...
        this.start_monitors = function() {
            if (!monitors) {
                ...
                _self.hm_monitor();
                if (!window._htmlvid&&!clicky_custom.video_disable&&!clicky_custom.video_auto_disable && (document.getElementsByTagName('audio').length || document.getElementsByTagName('video').length)) {
        ...
        this.goal = function(id, revenue, noq) {
            if (!id)
                return;
            var goal = (typeof id == 'number' || id.match(/^[0-9]+$/)) ? '[id]=' + id: '[name]=' + _self.enc(id);
            goal = {
                type: 'goal',
                query: '&goal' + goal + (revenue ? '&goal[revenue]=' + revenue : '')
            };
            if (noq ||!_self.queue_add(goal))
                _self.beacon('goal', goal);
        };
        this.beacon = function(type, q, called_by_pageview) {
            ...
            if (typeof q == 'object') {
                ...
                if (q.type == 'goal' && q.query) {
                    ...
                    var temp = '';
                    for (var i in q) {
                        if (i != 'type' && q.hasOwnProperty && q.hasOwnProperty(i))
                            temp += '&' + i + '=' + _self.enc(q[i]);
                    }
                    q = temp;
                    delete temp;
                }
            }
            var jsuid = '', goal = '', split = '';
                    ...
                    for (var i in clicky_custom['split']) {
                        if (clicky_custom['split'].hasOwnProperty && clicky_custom['split'].hasOwnProperty(i)) {
                            if (i == 'goal' && typeof clicky_custom['split'].goal == 'object') {
                                ...
                                split += '&split[' + _self.enc(i) + ']=' + _self.enc(clicky_custom.split[i]);
                        }
                    }
                    clicky_custom.split = '';
                }
            ...
            for (var site_id_index = 0; site_id_index < site_ids.length; site_id_index++) {
                ...
                if (_self.get_cookie('no_trackyy_' + site_id))
                    continue;
                if (_self.get_cookie('unpoco_' + site_id) && type != 'pageview')
                    continue;
            ...
            if (type == 'outbound' || type == 'download')
                _self.pause();
            _self.ref = '';
            _self.utm = '';
            _self.ping_start();
        ...
        this.ping_start = function() {
            if (clicky_custom.ping_disable || _self.pinging)
                return;
            _self.pinging = 1;
        ...
        this.get_cookie = function(name) {
            if (clicky_custom.sticky_data_disable && name.match(/^_(custom|utm|referrer)/))
                return '';
        ...
        this.set_cookie = function(name, value, expires) {
            if (clicky_custom.cookies_disable || (clicky_custom.sticky_data_disable && name.match(/^_(custom|utm|referrer)/)))
                return false;
            var ex = new Date;
        ...
        this.advanced = function() {
            ...
            for (var i = 0; i < a.length; i++) {
                ...
                if (a[i].className.match(/clicky_log/i)) {
                    ...
                    if (clicky_custom.outbound_disable || clicky_custom.advanced_disable || window.clicky_advanced_disable)
                        continue;
...
if (window.clicky_site_ids) {
    ...
    while (clicky_site_ids.length)
        clicky.init(clicky_site_ids.shift());
}
var _genericStats = clicky, _genericStatsCustom = clicky_custom;
